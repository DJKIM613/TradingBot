...
# 		if not check_table_exist(self.strategy_name, 'stock_info'):
...
# 					stock_info[code] = code_name
#
# 			# 코드, 종목명, 생성일자자를 열로 가지는 DaaFrame 생성
# 			universe_df = pd.DataFrame({
# 				'code': stock_info.keys(),
# 				'code_name': stock_info.values(),
# 				'created_at': [now] * len(stock_info.keys())
# 			})
#
# 			# universe라는 테이블명으로 Dataframe을 DB에 저장함
# 			insert_df_to_db(self.strategy_name, 'stock_info', universe_df)
#
...
# 		for idx, code in enumerate(self.stock_info.keys()):
# 			print("({}/{}) {}".format(idx + 1, len(self.stock_info), code))
#
...
# 					self.stock_info[code]['price_df'] = price_df
#
# 	def run(self):
# 		"""실질적 수행 역할을 하는 함수"""
...
#
# 				for idx, code in enumerate(self.stock_info.keys()):
# 					print('[{}/{}_{}]'.format(idx + 1, len(self.stock_info), self.stock_info[code]['code_name']))
# 					time.sleep(0.5)
#
# 					# (1)접수한 주문이 있는지 확인
...
#
# 		# stock_info 딕셔너리의 key값들은 종목코드들을 의미
# 		codes = self.stock_info.keys()
#
...
# 		"""매도대상인지 확인하는 함수"""
# 		universe_item = self.stock_info[code]
#
